...
static void* hypervisor_thread(void* guest_img_name){
  ...
  if (ioctl(v.vcpu_fd, KVM_SET_SREGS, &sregs) < 0) {
	...
	// PC se preko pt[0] ulaza mapira na fizičku adresu GUEST_START_ADDR (0x1000).
				...
				if(v.run->io.port == PORT_IO){
					...
					if (v.run->io.direction == KVM_EXIT_IO_OUT ) {
						...
						*(p + v.run->io.data_offset) = STATUS_INVALID;
						...
						printf("KVM_EXIT_IO file not io_in nor ion_out\n");
					}
...
// Обезбедити да гост може да отвори, затвори, чита и уписује у фајл. Потпис споменутих функција треба
// сам студент да осмисли тако да личе на потписе функција у језику C. Потребно је обезбедити:
// • Отварање, затварање, читање и уписивање у фајл је потребно реализовати преко IN/OUT
// инструкција користећи IO паралелни порт 0x0278. Отварање фајла који не постоји за упис
// креира тај фајл.
// • Задавање путање до фајла које гост може да користи за упис/читање. Путање се задају као
// параметар командне линије хипервизора преко опције -f или --file. Задати фајлови преко
// ове опције су дељени између виртуелних машина. У случају да нека ВМ покуша да упише у
// ове дељене фајлове, хипервизор мора да направи локалну копију за ту ВМ.
// Хипервизор мора да обезбедни правилно руковање фајловима, односно хипервизор мора да води
// рачуна да сваки гост приступа само фајловима које је тај гост направио или којима има приступ за
// читање.
